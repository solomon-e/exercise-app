...
import { useSignup } from '../hooks/useSignup' // import signup hook
...
const Signup = () => {
  ...
  // isLoading is used to disable singup button
  ...
  const handleSubmit = async (e) => {
    ...
    // recieves name email and password from form
  ...
  return (
    <div className='w-full max-w-md mx-auto rounded items-center m-4 p-4'>
      <form onSubmit={handleSubmit} className='bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 border border-black'>
        <div className='mb-4'>
          <h1 className='font-semibold text-lg md:text-2xl'>Sign Up</h1>
          <h3 className='mt-1 font-normal text-sm md:text-base text-gray-500'>Enter your details to register.</h3>
        </div>
        <div className='mb-4'>
          <label className='block text-gray-700 text-xs md:text-sm font-medium mb-2'>Username</label>
          <input
            type='text'
            className='text-sm md:text-base shadow border border-black rounded w-full py-2 px-3 text-gray-700 focus:outline-none'
